...
export function UploadClient({ creatorName }: { creatorName: string }) {
  ...
  async function submit() {
    ...
    // Step 1: POST metadata only — no file bytes, no thumbnail data URL.
    // Thumbnails are generated by Mux automatically; sending them through
    // Vercel functions would hit the 4.5 MB payload limit.
    ...
    // Progress here reflects the real upload to Mux CDN — no server hop.
    if (muxUploadUrl) {
      try {
